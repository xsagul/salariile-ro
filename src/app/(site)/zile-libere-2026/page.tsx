...
function buildMonth(m: number): LunaCalendar {
  const daysInMonth = new Date(Date.UTC(YEAR, m + 1, 0)).getUTCDate();
  const start = dowMonday(YEAR, m, 1);
  const cells: CelulaZi[] = Array.from({ length: start }, () => null);
  let lucr = 0;
  for (let d = 1; d <= daysInMonth; d++) {
    const weekend = dowMonday(YEAR, m, d) >= 5;
    const name = HOLIDAYS[`${m + 1}-${d}`];
    cells.push({ day: d, weekend, name });
    if (!weekend && !name) lucr++;
  }
  return { luna: m + 1, nume: LUNI_NUME[m], cells, lucr, libere: daysInMonth - lucr };
}
      ...
      <div className="bg-canvas">
        <div className={`mx-auto max-w-6xl px-4 sm:px-6 ${SPATIU_SUS} ${SPATIU_JOS}`}>
          ...
          <div className={`${SEPARATOR_SECTIUNE} md:col-span-5`}>
            ...
            <CalendarAn an={YEAR} luni={MONTHS} dataBuild={new Date().toISOString()} />
          </div>
          <aside className="mt-8 md:col-span-2 md:col-start-4 md:row-start-1 md:mt-6 md:self-start">
            <UrmatoareaZiLibera an={YEAR} dataBuild={new Date().toISOString()} />
          </aside>
          </div>

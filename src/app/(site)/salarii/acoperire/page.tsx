...
export default function Acoperire() {
  ...
  return <div className="bg-canvas"><div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
    ...
    <Lead>{total.toLocaleString('ro-RO')} anunțuri eligibile după verificare și deduplicare. Vezi unde există observații și unde avem încă lipsuri.</Lead>
    ...
    <div className="mt-6 max-w-3xl space-y-3 text-sm leading-relaxed text-stone-700">
      <p>Păstrăm anunțuri cu ocupație identificabilă, normă întreagă, muncă în România și sumă explicită, precizată ca net sau brut. Convertim brutul în net prin calculul fiscal standard și euro în lei la cursul de referință BCE datat. Registrul păstrează suma originală și fiecare conversie. Comisioanele fără salariu de bază, pachetele care includ beneficii fără defalcare și remunerațiile pe oră rămân în afara comparației.</p>
    ...
    <div className="mt-8 overflow-x-auto"><table className="w-full text-sm">
      ...
      <tbody>{rows.map(({m,a})=><tr key={m.slug} className="border-b border-stone-200"><th scope="row" className="p-3 text-left font-medium"><Link className="inline-flex min-h-11 items-center underline" href={`/salarii/${m.slug}`}>{m.nume}</Link></th><td className="p-3 text-right">{a?.n ?? 0} / {a?.explicitMonthly ?? 0}</td><td className="p-3 text-right">{a?.employers ?? 0}</td><td className="p-3 text-right">{a?.counties ?? 0}</td><td className="p-3 text-stone-600">{a ? Object.entries(a.sourceCounts).map(([s,n])=>`${s}: ${n}`).join(' · ') || '—' : '—'}</td><td className="p-3 text-stone-600">{a?.medianBounds ? 'Limite calculabile' : a?.n ? 'Acoperire insuficientă' : 'Fără anunțuri eligibile'}</td></tr>)}</tbody>
    </table></div>
  </div></div>;
}

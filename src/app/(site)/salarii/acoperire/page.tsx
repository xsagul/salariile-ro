...
export default function Acoperire() {
  ...
  return <div className="bg-canvas"><div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
    ...
    <div className="mt-6 max-w-3xl space-y-3 text-sm leading-relaxed text-stone-700">
      ...
      <p>Nu orice mențiune de net sau brut are aceeași greutate. Cea mai tare este cuvântul scris lângă sumă. Urmează cea aflată în altă parte a aceluiași anunț. Cea mai slabă este eticheta pe care un portal o pune identic pe toate anunțurile lui, din propriul model de date: la eJobs am găsit „net” pe 196 din 196 de anunțuri și niciodată „brut”, iar pagina nu conține nicio informație de bază per anunț. O numărăm, o folosim, dar o arătăm separat și nu o prezentăm drept declarație a angajatorului.</p>
    ...
    <div className="mt-8 overflow-x-auto"><table className="w-full text-sm">
      ...
      <tbody>{rows.map(({m,a})=><tr key={m.slug} className="border-b border-stone-200"><th scope="row" className="p-3 text-left font-medium"><Link className="inline-flex min-h-11 items-center underline" href={`/salarii/${m.slug}`}>{m.nume}</Link></th><td className="p-3 text-right">{a?.n ?? 0} / {a?.explicitMonthly ?? 0}</td><td className="p-3 text-right text-stone-600">{a?.basisNearAmount ?? 0}</td><td className="p-3 text-right text-stone-600">{a?.undeclaredBasis?.n ?? 0}</td><td className="p-3 text-right">{a?.employers ?? 0}</td><td className="p-3 text-right">{a?.counties ?? 0}</td><td className="p-3 text-stone-600">{a ? Object.entries(a.sourceCounts).map(([s,n])=>`${s}: ${n}`).join(' · ') || '—' : '—'}</td><td className="p-3 text-stone-600">{a?.medianBounds ? 'Limite calculabile' : a?.n ? 'Acoperire insuficientă' : 'Fără anunțuri eligibile'}</td></tr>)}</tbody>
    </table></div>
  </div></div>;
}

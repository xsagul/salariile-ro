...
- Catalogul are 142 de meserii (`MESERII.length` din `src/lib/meserii.ts` deține numărul). Plafonul cu cifră proprie din date oficiale e de ordinul a 500: verificat pe 31 august 2026, pe tot catalogul TEMPO (1.916 matrice), matricea **FOM121A** încrucișează activitatea cu grupa de ocupații, pe forme de proprietate, sexe și 11 ani — **544 de celule cu date, 527 de valori distincte**. Ce nu există nicăieri în TEMPO e COR: fiecare matrice de salarii cu dimensiune ocupațională are exact 10 opțiuni, Total plus cele 9 grupe majore ISCO. Deci „specialiști în servicii IT” rămâne o grupă, nu „programator”.
...
Aproximativ **3.400 de sesiuni pe lună includ cel puțin un calcul salarial**, măsurat în Umami în august 2026. Cifra nu se mai poate reface: instanța Umami a fost dezafectată pe 28 august 2026. Evenimentul `calcul` din GA4 (din 18 septembrie 2026) e alt instrument, cu altă populație — vezi mai jos, la măsurare — deci nu continuă seria Umami. Acesta este semnal de interes, nu echivalentul a 3.400 de persoane unice și nici un set de salarii declarat prin sondaj.
...
- **Publicarea se face automat prin Cloudflare Workers Builds**, conectat la
  repo-ul GitHub `xsagul/salariile-ro`, branch-ul `main`, din 22 septembrie 2026.
  Configurația din Cloudflare rulează `npm run build`, apoi
  `npx wrangler deploy`, din rădăcina repo-ului. Tokenul de build este administrat
  de Cloudflare. `.github/workflows/ci.yml` verifică testele, lint-ul, build-ul și
  HTML-ul randat, dar nu mai publică separat. Fallback-ul manual rămâne
  `npm run deploy` din rădăcină, într-un terminal autentificat în Cloudflare.
...
- Nicio cale de trafic viu nu trece prin Vercel. Planul, comutarea, măsurătorile și abaterile de la plan: `docs/arhiva/MIGRARE-CLOUDFLARE-2026-09-12.md` și `PROGRES.md` la 12 septembrie 2026.
...
Lista vie a rutelor publice e `src/app/(site)/`, un director pe rută; nu o copia aici.
Homepage-ul e calculatorul salariu net/brut, `/salarii/*` sunt paginile de meserie,
`/noutati` e secțiunea editorială, iar `/widget/*` servește widgeturile pentru
site-uri terțe.
...
- P0 a fost publicat după 24 iulie 2026, ultima zi din acest snapshot; efectul lui se citește din `npm run gsc:weekly`, nu din cifrele de mai sus
...
- GSC este conectat. **Măsurarea de bază rămâne Cloudflare Web Analytics, din 12 septembrie 2026** — fără cookies, fără localStorage și fără amprentare. Din 17 septembrie 2026 rulează și **Google Analytics 4 (`G-2L1J64H5H9`) prin Consent Mode**, legat de mesajul european standard publicat în AdSense: stocarea este `denied` implicit, înainte de acord pot exista pinguri fără cookies, iar alegerea poate fi schimbată din „Setări cookies”. Scriptul AdSense este prezent doar pentru CMP: Auto ads e oprit și nu există unități de anunț create manual. Testul de RPM de o zi, cerut de proprietar pe 21 septembrie 2026, e documentat în `PROGRES.md`; ziua cu reclame nu s-a abătut de la variația obișnuită în timp activ, afișări și calcule per utilizator. Google Signals și personalizarea reclamelor sunt oprite în configurația GA4. Rutele `/widget/frame*` nu primesc nici AdSense, nici GA4. **Din 18 septembrie 2026 GA4 are evenimente proprii**: `calcul` (eveniment cheie, pe toate cele 8 calculatoare), scroll 25/50/75, secțiuni văzute, ieșire cu timp vizibil și scroll maxim, dimensiunea ferestrei, căutări în filtrul de meserii, clickuri interne, Core Web Vitals și erori JS. Lista și parametrii au un singur proprietar: `src/lib/analytics.ts`. **Test A/B/C pe bara de sus, 24 septembrie – 8 octombrie 2026** (a = ca înainte, b = lipită sus, c = ascunsă la derulare în jos): varianta vine din cookie-ul `_ga`, deci doar vizitatorii cu acord intră în test; în GA4 se filtrează după `navbar_varianta` (eveniment) și `navbar_varianta_utilizator`. Nu schimba bara de sus până la final; după 8 octombrie se compară timpul activ, derularea, calculele, paginile pe sesiune și clickurile `zona` = `header`/`meniu_mobil`, separat pe telefon și desktop, apoi se păstrează o singură variantă. **Afișările de pagină se trimit din cod, cu `?brut=`/`?net=` scoase**, iar în GA4 Admin „modificări de pagină pe baza istoricului browserului” e OPRIT: până atunci, fiecare calcul trimitea suma salariului la Google într-o afișare falsă. Nu reporni setarea și nu trimite sume în evenimente — `scripts/test-ui-contracts.mts` pică la orice apel care pare să poarte o sumă. Vizitatorii fără acord trimit pinguri fără cookies: apar în evenimente și în timp real, dar fără sursă de trafic („(not set)” / „(data not available)”, canalele „Unassigned” / „Cross-network”) și fără sesiuni implicate — observat pe 17–18 septembrie 2026. Sursa și rata de implicare din GA4 descriu deci doar vizitatorii cu acord; volumele totale rămân în Cloudflare, sursele de căutare în Search Console. Core Web Vitals din teren se iau din CrUX (`npm run psi`) și se compară cu `docs/arhiva/CWV-INAINTE-DE-CLOUDFLARE-2026-09-12.json`.
- **Înainte de a adăuga ceva cu cotă sau facturat pe cerere** (un serviciu de măsurare, un script de Worker, middleware), estimează volumul din traficul real, nu din impresie: pe Vercel, Speed Insights s-a umplut la ~830 de afișări pe zi, iar middleware-ul se factura la fiecare cerere care îi atingea matcher-ul, boți incluși. Ce nu depinde de cerere stă în configurație (`_headers`, generat de `scripts/genereaza-cloudflare.mts`). Cifrele: `docs/arhiva/MIGRARE-CLOUDFLARE-2026-09-12.md`.
- **Link-urile nu se pre-încarcă.** `next/link` pre-încarcă în producție fiecare rută statică al cărei link intră în ecran, segment cu segment. Măsurat pe 11 septembrie 2026, vizitator nou pe mobil, cu scroll complet: homepage 143 de cereri pe vizită, din care 113 pre-încărcări; după oprire, 17, indiferent de scroll, cu navigarea prin click tot client-side. Toate link-urile trec prin `src/app/components/Link.tsx` (`prefetch = false`), iar `scripts/test-ui-contracts.mts` pică la orice import direct din `next/link`. Motivul nu e costul de găzduire — pe Cloudflare cererile de assets sunt gratuite — ci vizitatorul: 113 cereri inutile pe vizită înseamnă baterie și trafic mobil consumate degeaba, pe seama vizitatorului.
...
- **Persistență:** salvează progresul în `PROGRES.md` la finalul fiecărei etape, ca sesiunea următoare să continue de unde ai rămas.
...
- **Niciun an vechi în prima parte a paginii.** Decis de proprietar pe 15 septembrie 2026: un vizitator neatent care vede „Legea 153/2017" sau „din 2025" lângă titlu crede că pagina e învechită și pleacă instant. Sus se scrie „în vigoare" / „în plată"; numărul actului cu anul lui stă lângă surse, mai jos. Pe paginile-instrument regula e verificată de `scripts/test-rendered.mts`.
...
Calculatorul rămâne pe homepage până sunt îndeplinite toate cele nouă condiții din `ROADMAP-90-ZILE.md`, secțiunea 12. Cele două ferestre post-P0 de câte 28 de zile sunt doar prima dintre ele. Până atunci facem doar adăugări și optimizări non-distructive.

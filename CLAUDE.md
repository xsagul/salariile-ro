...
- **`main` NU e codul servit în producție.** Codul live stă pe ramura `migrare-cloudflare`: `next.config.ts` cu export static, `wrangler.jsonc`, `src/lib/csp.ts` (singurul proprietar al CSP + `Link`), `_headers` și `_redirects` generate de `scripts/genereaza-cloudflare.mts`. Pe `main` încă există `src/proxy.ts`, `src/app/api/**` și `src/app/info/`, toate șterse pe ramura de migrare. Publicarea se face manual cu `wrangler`, până când proprietarul pune cheile în GitHub.
...
- **Ramura `migrare-cloudflare` rămâne neunită în `main`, deliberat.** Vercel e încă legat de repo: un merge ar declanșa acolo un build al exportului static fără headere de securitate și fără redirecturi. Unirea se face abia după ce proprietarul scoate domeniul din Vercel.
